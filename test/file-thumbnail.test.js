...
const POSTER2 = '_thumbs/7c9e6679-7425-40de-944b-e07fc1f90ae7.poster.jpg';
...
test('an upload is recorded with its kind, thumbnail and media facts, and a later thumbnail merges', { skip: !live && 'TEST_DATABASE_URL not reachable' }, async (t) => {
  ...
  // Back-date the row, so a write that stamps "now" cannot pass unnoticed.
  ...
  assert.equal(after.updatedAt, 1000, 'nor a modification: Modified and Finder’s date stay put');
...
test('a player poster is recorded with its thumbnail, replaced with it, and cleared by one without', { skip: !live && 'TEST_DATABASE_URL not reachable' }, async (t) => {
  const body = {
    name: 'poster.mp4', url: 'http://s3.test/b/files/poster.mp4', mime: 'video/mp4', size: 1000,
    storage: 's3', storageKey: 'files/poster.mp4', thumbnailKey: KEY, posterKey: POSTER,
    media: { width: 3840, height: 2160, duration: 12 },
  };
  const file = await db.createFile({ ...body, ...uploadFields(body), createdBy: 'test@example.com' });
  t.after(async () => { await db.sql`DELETE FROM files WHERE id = ${file.id}`.catch(() => {}); });
  assert.equal(file.posterKey, POSTER);
  assert.equal((await db.getFileById(file.id)).posterKey, POSTER);

  // While a row points at them, the old previews are not deletable.
  assert.deepEqual(await db.unreferencedPreviewKeys({ thumbKeys: [KEY], posterKeys: [POSTER] }), []);

  const after = await db.setFileThumbnail(file.id, KEY2, { width: 3840, height: 2160 }, POSTER2);
  assert.equal(after.thumbnailKey, KEY2);
  assert.equal(after.posterKey, POSTER2);
  // Replaced, nothing points at the old pair any more — unless another row does.
  assert.deepEqual((await db.unreferencedPreviewKeys({ thumbKeys: [KEY], posterKeys: [POSTER] })).sort(), [KEY, POSTER].sort());
  assert.deepEqual(await db.unreferencedPreviewKeys({ thumbKeys: [KEY2], posterKeys: [POSTER2] }), []);

  const cleared = await db.setFileThumbnail(file.id, KEY, {});
  assert.equal(cleared.posterKey, null, 'a new thumbnail without a poster does not keep a poster of another frame');
  assert.equal((await db.getFileById(file.id)).posterKey, null);
});
