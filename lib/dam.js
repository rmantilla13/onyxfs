...
import { effectiveKind, MEDIA_KEYS } from './media.js';
...
export function validateMetadataPatch(patch, schema) {
  ...
  for (const [key, val] of Object.entries(patch)) {
    ...
    // The rest of the media's own facts — the frame model, above all — are
    // written only by the server, from what it read (lib/media.js), even if
    // a schema from before they were reserved has a field of that name.
    if (MEDIA_KEYS.includes(key)) continue;
    const f = byKey[key];
    if (!f) continue;
...
export function addMetadataField(schema, { label, type = 'text', options } = {}) {
  ...
  const reserved = new Set(['tags', ...MEDIA_KEYS.map(slug), ...AUTO_FACETS.map((a) => a.key)]);
