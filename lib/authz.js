...
// the answer was spread across places that disagreed: checks of one literal
// role id, an env-only isAdmin, and a desktop-only
...
import {
  parseRolesConfig, resolveRole, roleCaps, capDriveRole, isCapability, READ_ONLY_ROLE,
} from './roles.js';
import { parsePolicy, effectiveLimits } from './policy.js';
import { mergeFlags, applyBetaAdminFlags, DEFAULT_FLAGS } from './features.js';
import { drivePatterns, driveAccess, drivesHolding, DRIVE_WRITE_ROLES } from './drive-access.js';
...
export function principalFrom({
  ...
  let role = resolveRole(e, rolesConfig, { isAdmin, roleId: person?.roleId || null, hasRow: !!person });
  if (degraded && !isAdmin) role = { ...READ_ONLY_ROLE, legacyAdmin: !!role.legacyAdmin, degradedFrom: role.id };
  ...
  const principal = {
    ...
    tokenId,
    person,
    role: { id: role.id, name: role.name, builtin: !!role.builtin, ...(role.degradedFrom ? { degradedFrom: role.degradedFrom } : {}) },
    // What role-subject grants (folder_access, file_acl) are matched against.
    // A degraded principal borrows the read-only role's CAPABILITIES, never
    // its identity: matching grants made to the Viewer role would hand every
    // Member whatever those grants reach, during exactly the outage in which
    // nothing can be checked. No role at all instead — their own grants
    // still count, role grants wait for a real answer. Fewer, never other.
    roleId: degraded && !isAdmin ? null : role.id,
...
export async function getPrincipal(email, { tokenId = null, person } = {}) {
  ...
  if (!admin) {
    ...
    // after it — and there are none while degraded, when roleId is null. A
    // failure here is fewer grants, never more.
...
function shareDecision(p, action, r) {
  ...
  if (Array.isArray(r.driveShareKinds) && !r.driveShareKinds.includes(kind)) {
    return deny(r.driveShareKinds.length
      ? `This drive does not allow ${LINK_LABEL[kind] || kind} links.`
      : 'Links are turned off for this drive.');
  }
...
export function can(principal, action, resource = {}) {
  ...
  if (!principal.isAdmin && !(principal.caps instanceof Set && principal.caps.has(action))) {
    // "Private links only" is only true for someone who can make those.
    if (action === 'shares.public' && !principal.caps?.has?.('shares.private')) return deny(NO_CAP['shares.private']);
...
export function mountRole({ driveRole, canUpload, overQuota = false }) {
  ...
  // Anything but a write role — viewer, or a value nobody defined — mounts read-only.
  if (!DRIVE_WRITE_ROLES.has(driveRole) || !canUpload || overQuota) return 'viewer';
  return driveRole;
}
