...
//                   and forget upload keys issued but never recorded
...
import {
  ...
  pruneUploadKeys, UPLOAD_KEY_TTL_MS,
} from './db.js';
 ...
 * { id, ok, schema, purged, purgeErrors, uploadsAborted, uploadKeysPruned,
 *   expired, soon, auditPruned, errors, durationMs }. `ok` is false when a job threw or a
...
export async function runMaintenance({ trigger = 'cron', by = null } = {}) {
  ...
  const out = {
    schema: [], purged: 0, purgeErrors: 0, uploadsAborted: 0, uploadKeysPruned: 0, expired: 0, soon: 0, auditPruned: 0, errors: [],
  };
  ...
  // Past their TTL a key cannot be recorded anyway (claimUploadKey); this
  // only keeps the table from growing by one row per abandoned upload.
  await step('uploadKeys', async () => { out.uploadKeysPruned = await pruneUploadKeys(Date.now() - UPLOAD_KEY_TTL_MS); });
