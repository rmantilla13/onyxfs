...
import {
  getSetting, getFilespaceById, grantFilespaceAccess, revokeFilespaceAccess, isFilespaceRole,
} from './db.js';
...
export function presentPerson(row, { rolesConfig, policy, adminEmails = getAdminEmails() }) {
  ...
  const role = resolveRole(email, rolesConfig, { isAdmin: envAdmin, roleId: person?.roleId || null });
