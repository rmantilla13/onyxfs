...
/** An endpoint as the service it names: no scheme, no trailing slash, any case. Blank is AWS. */
export const endpointHost = (endpoint) => String(endpoint || '').trim().toLowerCase()
  .replace(/^[a-z][a-z0-9+.-]*:\/\//, '').replace(/\/+$/, '');
const hostOf = endpointHost;
