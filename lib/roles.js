...
// below, less anything the saved copy had turned off; a custom role keeps
// its name and has its old `features` map translated into capabilities.
//
...
// a row is born with their v1 assignment copied onto it (upsertPerson), so
// from then on the row alone decides, and an admin who sets someone back to
// the default role gets exactly that.
...
export const BUILTIN_ROLES = [
  ...
  // The stored id stays `viewer`: ids are never renamed.
  {
    id: 'viewer',
    name: 'Viewer',
    description:
...
/**
 * The read-only built-in, by what it is rather than by its id: what a
 * principal falls back to when its role could not be read (lib/authz.js).
 * Code asks what a role may DO, never which role it is.
 */
export const READ_ONLY_ROLE = BUILTIN_ROLES[BUILTIN_ROLES.length - 1];
...
function normalizeRole(r, { v1 }) {
  ...
  // A v1 copy of a built-in is the old default, saved — plus whatever an
  // admin took away from it, which is still theirs to have taken. So the new
  // default, with the saved copy's refusals translated and laid over it: a
  // Member someone had barred from sharing stays barred at deploy. It can
  // only take away, never restore something the new default leaves out.
  if (builtin && v1) {
    const role = clone(builtin);
    role.caps = { ...role.caps, ...capsFromFeatures(r.features) };
    return role;
  }
...
/**
 * The role for an email. Order: the person's own row (`roleId`), then — only
 * for someone with no row yet (`hasRow` false) — v1's assignments map, then
 * the default. A row's NULL role means the default role, full stop: the row
 * was born with any v1 assignment already copied onto it, so reading the map
 * again would undo an admin who has since set them back to the default.
 * Env admins get ADMIN_ROLE.
 *
 ...
 */
export function resolveRole(email, config, { isAdmin = false, roleId = null, hasRow = false } = {}) {
  if (isAdmin) return { ...clone(ADMIN_ROLE), legacyAdmin: false };
  // Always through the parser, even for a config that looks parsed: it is
  // idempotent and cheap, and a stored blob that merely says version 2 is
  // not proof its roles were normalized.
  const cfg = parseRolesConfig(config);
  const e = String(email || '').trim().toLowerCase();
  let id = roleId || (hasRow ? null : cfg.assignments[e]) || cfg.defaultRole;
...
export function validateRolesConfig(input, current) {
  ...
  // v1 assignments to a role this save deletes go with it. They only matter
  // for someone who has never arrived (everyone else's is on their row), and
  // that person should arrive in the default role — not in whatever role
  // someone later creates under the same id.
  const assignments = Object.fromEntries(Object.entries(stored.assignments).filter(([, id]) => !removed.includes(id)));
  return {
    config: { version: 2, defaultRole, roles, assignments, legacyFullIds: stored.legacyFullIds },
    removed,
  };
}
