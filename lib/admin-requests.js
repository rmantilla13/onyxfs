...
/**
 * Why an approved person's access cannot be revoked here, in the words
 * shown in place of the button — or null when it can. A disabled button
 * with its reason in a tooltip says nothing on a phone and cannot be
 * focused, so the reason is the text.
 */
export function revokeBlockedReason(row) {
  if (row?.self) return 'This is you';
  if (row?.envAdmin) return 'Admin, set in ADMIN_EMAILS';
  return null;
}
