 ...
 *
 * A frame's start time is n·den/num exactly, and the float product lands a
 * hair under the integer often enough (1001/24000 · 24000/1001 is
 * 0.9999999999999999) that floor would read it as the frame before. So it is
 * nudged by a millionth of a frame — far smaller than any real timing
 * difference and far larger than float error, even ten hours in at 120fps.
 */
export function frameAt(sec, fps) {
  const t = Number(sec);
  if (!Number.isFinite(t) || t <= 0) return 0;
  const r = toRate(fps) || ASSUMED_RATE;
  return Math.floor((t * r.num) / r.den + 1e-6);
}
