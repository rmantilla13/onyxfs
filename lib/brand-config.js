...
export function defaultBrandConfig() {
  ...
  return {
    ...
    lockupDarkUrl: BRAND.visual.logo.lockupDarkPath,
...
/**
 * The logo, white-label safe. Our wordmark spells our name, so it is shown
 * only while the deployment is still ours: one that renamed itself or set its
 * own mark keeps that mark and its name (lockupPath null — see BrandLogo)
 * until it supplies a wordmark of its own. A wordmark with no dark version is
 * used in both schemes. A saved value equal to our default counts as unset,
 * so a form that writes every field back cannot switch the wordmark off.
 */
function resolveLogo(c, d) {
  const own = (key, fallback) => {
    const v = str(c[key]);
    return v && v !== fallback ? v : null;
  };
  const dl = d.visual.logo;
  const lockup = own('lockupUrl', dl.lockupPath);
  const lockupDark = own('lockupDarkUrl', dl.lockupDarkPath);
  const ours = !own('name', d.name) && !own('markUrl', dl.markPath);
  return {
    markPath: str(c.markUrl) || dl.markPath,
    lockupPath: lockup || (ours ? dl.lockupPath : null),
    lockupDarkPath: lockupDark || lockup || (ours ? dl.lockupDarkPath : null),
    // Our own wordmark is drawn inline (OnyxWordmark), which is what lets its
    // letters take the theme's ink and its "/FS" the brand's colours.
    onyxWordmark: !lockup && ours,
  };
}
...
const EDITABLE = new Set([
  ...
  'markUrl', 'lockupUrl', 'lockupDarkUrl', 'radius',
