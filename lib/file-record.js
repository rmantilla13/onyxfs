...
export function parseFileRecord(body) {
  ...
  const record = {
    ...
    // Checked by uploadFields: a thumbnail, player poster or strip key the
    // presign route named, media facts that are sane numbers, a strip
    // geometry a browser can decode.
    thumbnailKey: str(body.thumbnailKey, 2048),
    posterKey: str(body.posterKey, 2048),
