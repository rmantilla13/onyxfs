...
// the sign-in gate and to canAccessFile first, so a comment's words never
// reach someone who has since lost access: to the workspace (a ban, a removed
// account), or to the file (a drive they left, a grant that was revoked).

import {
  createNotification, listReviewWatchers, getFeatureFlags, buildPrincipal, canAccessFile,
} from './db.js';
import { isEmailGrantedAccess } from './auth-allowlist.js';
...
// A watcher is remembered from the day they commented or were named; whether
// they may still sign in, and still read the file, is asked now.
async function canRead(file, email) {
  try {
    if (!(await isEmailGrantedAccess(email))) return false;
    return await canAccessFile(file, await buildPrincipal(email));
  } catch { return false; }
}
