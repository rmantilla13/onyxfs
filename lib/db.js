 ...
 * applies only to a row this call creates.
 */
export async function upsertPerson(email, { seen = false, roleId = null } = {}) {
  ...
  const rows = await withSchemaRetry(ensurePeopleTable, () => sql`
    ...
    VALUES (${crypto.randomUUID()}, ${e}, ${roleId || null}, ${at}, ${at}, ${now}, ${now})
...
export async function personDetail(email) {
  ...
  const [[head], drives, links, devices, [usage]] = await Promise.all([
    sql`
      SELECT p.*, i.status AS invite_status, i.name AS invite_name,
             i.reviewed_by AS invite_reviewed_by, i.reviewed_at AS invite_reviewed_at,
...
export async function countPeopleByRole(defaultRole = 'member') {
  if (!sql) return {};
  await ensurePeopleTable();
  const rows = await sql`SELECT COALESCE(role_id, ${defaultRole}) AS role_id, COUNT(*)::int AS n FROM people GROUP BY 1`;
...
export async function clearRoleAssignments(roleIds = []) {
  ...
  await ensurePeopleTable();
  const rows = await sql`UPDATE people SET role_id = NULL, updated_at = ${Date.now()} WHERE role_id = ANY(${roleIds}::text[]) RETURNING id`;
  return rows.length;
}
...
export async function pruneAuditEvents(olderThanMs) {
  if (!sql) return 0;
  await ensureAuditTable();
  const rows = await sql`DELETE FROM audit_events WHERE at < ${Number(olderThanMs)} RETURNING id`;
  return rows.length;
}
 ...
 * height and duration it read on the way. Metadata is merged, as updateFile
 * does. `version` is left alone: a poster is not an edit, and bumping it would
 * fail someone else's If-Match for a change they cannot see.
 */
export async function setFileThumbnail(id, thumbnailKey, media = {}) {
  if (!sql) throw new Error('Database not configured');
  await ensureFilesTable();
  const rows = await sql`
    UPDATE files
...
export async function getTrashedFiles(ids = []) {
  ...
  await ensureFilesTable();
  const rows = await sql`SELECT * FROM files WHERE id = ANY(${list}::text[]) AND deleted_at IS NOT NULL`;
  return rows.map(shapeFile);
}
...
export async function deleteShares(tokens = []) {
  ...
  if (!sql || !list.length) return [];
  await ensureSharesTable();
  const rows = await sql`DELETE FROM file_shares WHERE token = ANY(${list}::text[]) RETURNING token, file_id, created_by`;
 ...
 * for an exact match.
 *
 * `driveRole` is their (ceiling-capped) role in the drive the folder is in,
 * when the operation is scoped to one. A drive's editors and owners
 * restructure its folders: their desktop mount already lets them, and the
 * web used to refuse them with a 403 because only folder grants counted.
 * Whether their ROLE lets them manage folders at all is the caller's
 * question (can(principal, 'folders.manage')).
 */
export async function folderRoleFor(folder, principal = {}, { driveRole = null } = {}) {
  if (principal.isAdmin) return 'owner';
  const fromDrive = WRITE_ROLES.has(driveRole) ? driveRole : null;
  if (!sql) return fromDrive;
  const email = String(principal.email || '').toLowerCase();
  const roleId = principal.roleId || null;
  try {
    await ensureFolderAclTable();
...
export async function updateFilespace(id, fields = {}) {
  ...
  const shareKinds = Array.isArray(m.shareKinds) ? m.shareKinds.join(',') : null;
  const rows = await sql`
...
export async function countFilespacesCreatedBy(email) {
  ...
  if (!sql || !e) return 0;
  await ensureFilespacesTables();
  const rows = await sql`SELECT COUNT(*)::int AS n FROM filespaces WHERE lower(created_by) = ${e}`;
