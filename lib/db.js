...
import { mergeFlags } from './features';
...
// Build the client defensively. neon() throws synchronously on a missing or
// malformed connection string, and this module is imported by auth.js — the
// Auth.js root — so that throw would take down auth initialization and reach
// users as an opaque "Configuration" error page. Defer the failure to query
// time instead, where it lands in the logs with a message that says what to fix
// and /api/health can report it.
function createSqlClient() {
  ...
  try {
    ...
    console.error('[db] neon() init failed:', e.message);
...
export async function listSettings(prefix = '') {
  ...
  try {
    const rows = prefix
      ? await sql`SELECT key, value, updated_at, updated_by FROM settings WHERE key LIKE ${prefix + '%'} ORDER BY key`
      : await sql`SELECT key, value, updated_at, updated_by FROM settings ORDER BY key`;
...
let magicLinkRedirectsEnsured = false;
async function ensureMagicLinkRedirectsTable() {
  if (magicLinkRedirectsEnsured) return;
  ...
  try {
    ...
    magicLinkRedirectsEnsured = true;
  } catch (e) {
    console.warn('[ensureMagicLinkRedirectsTable] failed:', e.message);
  }
}
...
let inviteRequestsTableEnsured = false;
async function ensureInviteRequestsTable() {
  if (!sql) return;
  if (inviteRequestsTableEnsured) return;
  await sql`
  ...
  `;
  inviteRequestsTableEnsured = true;
}
...
let notificationsTablesEnsured = false;
async function ensureNotificationsTable() {
  if (!sql) return;
  if (notificationsTablesEnsured) return;
  await sql`
  ...
  `;
  notificationsTablesEnsured = true;
}
...
async function ensureFilesTable() {
  ...
  try {
    ...
    // Cue review minted from this file (so the "Review in Cue" action reuses one
    // review per video instead of spawning a new one on every click).
    await sql`ALTER TABLE files ADD COLUMN IF NOT EXISTS cue_review_id TEXT`;
    ...
    console.warn('[ensureFilesTable] failed:', e.message);
  }
}
...
function shapeFile(r) {
  ...
  return {
    ...
    cueReviewId: r.cue_review_id || null,
...
/** Link a file to the Cue review minted from it (one-time). */
export async function setFileCueReview(id, reviewId) {
  if (!sql) throw new Error('Database not configured');
  await ensureFilesTable();
  await sql`UPDATE files SET cue_review_id = ${reviewId} WHERE id = ${id}`;
  return { ok: true };
}
...
export async function createFile(data = {}) {
  ...
  const rows = await sql`
    INSERT INTO files (id, name, folder, kind, mime, size, url, storage, storage_key, tags, notes, visibility, thumbnail_url, thumbnail_key, metadata, created_by, created_at, updated_at)
    VALUES (
      ...
      ${JSON.stringify(Array.isArray(data.tags) ? data.tags : [])}, ${data.notes || null}, ${visibility},
...
export async function listFiles(opts = {}) {
  if (!sql) return { files: [], total: 0 };
  try {
    await ensureFilesTable();
    const { folder, folderPrefix, q, kind } = opts;
    const rows = await sql`SELECT * FROM files ORDER BY created_at DESC`;
    let out = rows.map(shapeFile);

    // Trash filter: hide soft-deleted files everywhere except an explicit trash view.
    out = out.filter((f) => (opts.trashed ? f.deletedAt : !f.deletedAt));

    // Hide our own generated thumbnail objects (they're previews, not files).
    // Earlier bucket syncs ingested them as standalone rows. Precise signal: a
    // row whose key is referenced as some file's thumbnail_key — plus anything
    // under the dedicated `_thumbs/` prefix.
    const thumbKeys = new Set(rows.map((r) => r.thumbnail_key).filter(Boolean));
    out = out.filter((f) => !_isThumbArtifact(f.storageKey) && !_isSystemKey(f.storageKey) && !thumbKeys.has(f.storageKey));

    // Filespace scope: keep only files whose object key lives under the given
    // bucket prefix (a filespace = a prefix). Empty/undefined = no filespace filter.
    if (opts.storagePrefix != null) {
      const sp = String(opts.storagePrefix).replace(/^\/+|\/+$/g, '');
      if (sp) out = out.filter((f) => f.storageKey && (f.storageKey === sp || f.storageKey.startsWith(sp + '/')));
    }

    if (folderPrefix !== undefined && folderPrefix !== null) {
      const p = String(folderPrefix);
      out = out.filter((f) => f.folder === p || (p === '' ? true : f.folder.startsWith(p + '/')));
    } else if (folder !== undefined && folder !== null) {
      out = out.filter((f) => f.folder === folder);
    }
    if (kind) {
      const kinds = Array.isArray(kind) ? kind : [kind];
      if (kinds.length) out = out.filter((f) => kinds.includes(f.kind));
    }
    if (Array.isArray(opts.tags) && opts.tags.length) {
      const want = opts.tags.map((t) => String(t).toLowerCase());
      const mode = opts.tagMode === 'any' ? 'any' : 'all';
      out = out.filter((f) => {
        const have = (f.tags || []).map((t) => String(t).toLowerCase());
        return mode === 'all' ? want.every((t) => have.includes(t)) : want.some((t) => have.includes(t));
      });
    }
    if (opts.createdAfter) out = out.filter((f) => (f.createdAt || 0) >= opts.createdAfter);
    if (opts.createdBefore) out = out.filter((f) => (f.createdAt || 0) <= opts.createdBefore);
    if (q) {
      const needle = String(q).toLowerCase();
      out = out.filter((f) =>
        f.name.toLowerCase().includes(needle) ||
        (f.tags || []).some((t) => String(t).toLowerCase().includes(needle)) ||
        (f.notes || '').toLowerCase().includes(needle) ||
        (f.caption || '').toLowerCase().includes(needle));
    }
    const sort = opts.sort || 'new';
    out.sort((a, b) =>
      sort === 'old' ? (a.createdAt || 0) - (b.createdAt || 0) :
...
export async function updateFile(id, fields = {}) {
  ...
  const rows = await sql`
    UPDATE files SET
      ...
      tags = COALESCE(${fields.tags !== undefined ? JSON.stringify(fields.tags) : null}, tags),
...
async function ensureFoldersTable() {
  ...
  try {
    ...
    foldersEnsured = true;
  } catch (e) { console.warn('[ensureFoldersTable] failed:', e.message); }
}
...
let sharesEnsured = false;
async function ensureSharesTable() {
  if (sharesEnsured) return;
  if (!sql) throw new Error('Database not configured');
  try {
    await sql`
    ...
    sharesEnsured = true;
  } catch (e) { console.warn('[ensureSharesTable] failed:', e.message); }
}
...
// ─────────────────────────────────────────────────────────────────────────
let folderAclEnsured = false;
async function ensureFolderAclTable() {
  if (folderAclEnsured) return;
  if (!sql) throw new Error('Database not configured');
  try {
    await sql`
    ...
    folderAclEnsured = true;
  } catch (e) { console.warn('[ensureFolderAclTable] failed:', e.message); }
}

let fileAclEnsured = false;
async function ensureFileAclTable() {
  if (fileAclEnsured) return;
  if (!sql) throw new Error('Database not configured');
  try {
    await sql`
    ...
    fileAclEnsured = true;
  } catch (e) { console.warn('[ensureFileAclTable] failed:', e.message); }
}
...
export async function listFilesForUser(opts = {}, principal = {}) {
  const { files, total } = await listFiles(opts);
  if (principal.isAdmin) return { files, total };
  const email = (principal.email || '').toLowerCase();
  const ids = files.map((f) => f.id);
  const [granted, folderGrants] = await Promise.all([
    fileAclMap(ids, principal),
    folderGrantsForPrincipal(principal),
  ]);
  const folderAllowed = (folder) => {
    if (!folderGrants.size) return false;
    // A grant on any ancestor (incl. '') of the file's folder allows it.
    return _ancestorsOf(folder).some((a) => folderGrants.has(a));
  };
  const visible = files.filter((f) => {
    if ((f.createdBy || '').toLowerCase() === email) return true; // owner
    if (f.visibility === 'org') return true;
    if (granted.has(f.id)) return true;
    if (folderAllowed(f.folder)) return true;
    return false;
  });
  return { files: visible, total: visible.length, totalUnfiltered: total };
}
...
export async function getFilespaceForUser(email, id) {
  if (!id) return null;
  let admin = false;
  try { const { isAdmin } = await import('./auth-allowlist'); admin = isAdmin(String(email || '').toLowerCase()); } catch {}
  if (!admin) {
...
export async function buildPrincipal(email) {
  ...
  try { const { isAdmin } = await import('./auth-allowlist'); admin = isAdmin(e); } catch {}
  try {
  ...
  return { email: e, isAdmin: admin, roleId };
}
...
// ─────────────────────────────────────────────────────────────────────────
let filespacesEnsured = false;
async function ensureFilespacesTables() {
  if (filespacesEnsured) return;
  if (!sql) throw new Error('Database not configured');
  try {
    await sql`
    ...
    filespacesEnsured = true;
  } catch (e) {
    console.warn('[ensureFilespacesTables] failed:', e.message);
  }
}
...
// ─────────────────────────────────────────────────────────────────────────
let desktopAuthEnsured = false;
async function ensureDesktopAuthTables() {
  if (desktopAuthEnsured) return;
  if (!sql) throw new Error('Database not configured');
  try {
    await sql`
    ...
    desktopAuthEnsured = true;
  } catch (e) {
    console.warn('[ensureDesktopAuthTables] failed:', e.message);
  }
}
...
// ── Project management (workflows, kanban, approvals) ──────────────────────
