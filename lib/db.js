...
//
// ── SCHEMA_MANAGED=1 ─────────────────────────────────────────────────────────
// With this set, the guards do nothing on the request path. The schema is
// then owned by db/init.sql (run once in the SQL editor) and by
// ensureSchema(), which the maintenance cron, /api/health and `npm run
// doctor` call explicitly.
//
// Set it in production once init.sql has been run. Without it every cold
// start — every new lambda instance, and bots hitting /signin make plenty —
// issues a few dozen DDL statements before its first real query. Each
// ALTER TABLE … ADD COLUMN IF NOT EXISTS takes an ACCESS EXCLUSIVE lock on
// its table even when it adds nothing, so concurrent cold starts convoy on
// each other's locks and everything else queues behind them. That is the
// wrong thing to be doing on a one-connection pool in front of a shared
// database, and it is a bad place to be when the database is slow anyway.
//
// A schema change after that is: add the DDL to the guard as before, add the
// same line to db/init.sql, and run it — or hit the cron endpoint, which
// runs every guard regardless of the flag.
const schemaManaged = () => /^(1|true|yes|on)$/i.test(process.env.SCHEMA_MANAGED || '');
const SCHEMA_GUARDS = [];
...
export async function getSetting(key, { deadlineMs = RENDER_DEADLINE_MS } = {}) {
  ...
  try {
    ...
    settingsCache.set(key, { value: null, expires: Date.now() + SETTINGS_FAIL_TTL_MS });
    return null;
  }
}
