...
export function describeConnection(raw) {
  let u;
  try { u = new URL(raw); } catch { return { label: 'unparseable connection string', warn: null }; }
  ...
  if (pooled && port === '6543') kind = 'Supabase pooler, transaction mode';
    ...
    warn = 'This bypasses the pooler: every invocation opens a real Postgres connection, and '
         + 'the database\'s connection limit is reached at low traffic — after which new '
         + 'connections wait rather than fail, which reads as every query timing out. '
         + 'Switch to the transaction pooler (host ...pooler.supabase.com, port 6543).';
  }
...
if (connectionString) {
  ...
  if (process.env.DATABASE_URL && process.env.POSTGRES_URL
    ...
    console.warn(
      ...
      + 'DATABASE_URL is being used; the integration-managed POSTGRES_URL is '
      + 'ignored and will keep drifting. Delete one so there is a single answer.',
    );
  }
}
