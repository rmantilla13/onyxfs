...
let uploadsEnsured = false;
async function ensureUploadsTable() {
  if (uploadsEnsured) return;
  if (!sql) throw new Error('Database not configured');
  try {
    await sql`
    ...
    console.warn('[ensureUploadsTable] failed:', e.message);
  }
}
...
let tombstonesEnsured = false;
async function ensureTombstonesTable() {
  if (tombstonesEnsured) return;
  if (!sql) throw new Error('Database not configured');
  try {
    await sql`
    ...
    tombstonesEnsured = true;
  } catch (e) {
    console.warn('[ensureTombstonesTable] failed:', e.message);
  }
}
...
export async function listFileChanges({ cursor = 0, limit = 500 } = {}) {
  ...
  try {
    ...
    const [rows, tombs] = await Promise.all([
      sql(`SELECT ${FILE_COLUMNS} FROM files WHERE seq > $1 ORDER BY seq ASC LIMIT $2`, [from, n]),
      sql('SELECT id, seq, folder, storage_key, deleted_at FROM file_tombstones WHERE seq > $1 ORDER BY seq ASC LIMIT $2', [from, n]),
    ]);
...
export async function listFilesForUser(opts = {}, principal = {}) {
  ...
  try {
    ...
    if (opts.withTotal) {
      const counted = await sql(countText, countParams);
