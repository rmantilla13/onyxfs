...
/**
 * Local development with no RESEND_API_KEY: the link is printed to the
 * `next dev` terminal instead of being emailed, so a fresh checkout can sign
 * in without a mail provider.
 *
 * Only under `next dev`. A production build never takes this path, so a
 * deployment that is missing its key still fails loudly at sign-in rather
 * than quietly writing working sign-in links into its logs.
 */
export function printsSignInLinks() {
