 ...
 */
async function putPart({ blob, partNumber, uploadId, signal, onProgress, onPartial }) {
  let lastError;
...
export async function uploadFileMultipart(file, {
  ...
  // Bytes of parts still in flight count too, so a file split into 8 MB
    ...
    for (let i = 0; i < pending.length; i += SIGN_BATCH) {
      ...
      await pool(
        slice.map((partNumber) => () => {
          ...
          return putPart({
            blob, partNumber, uploadId: id, signal,
