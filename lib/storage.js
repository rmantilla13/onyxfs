...
export async function s3Diagnostics(cfg, { origin = '' } = {}) {
  ...
  checks.push(pass('config', 'Configuration', `${limits.label} · bucket "${cfg.bucket}" · objects up to ${limits.maxObjectLabel}`));
  ...
  try {
    ...
    checks.push(fail('read', 'Read access', describeS3Error(e, provider),
      provider === 'b2'
        ? 'On B2 the master application key does not work with the S3 API — create a regular application key with "Allow List All Bucket Names".'
