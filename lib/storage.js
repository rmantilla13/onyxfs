...
export function credentialPlan(cfg, filespace, { role = 'viewer' } = {}) {
  ...
  // R2, MinIO, Spaces and friends have no STS endpoint at all, so there is no
  // scoped rung to fall to.
...
export function staticRefusalMessage() {
...
export async function mintFilespaceCredentials(cfg, filespace, opts = {}) {
  ...
  const staticOrRefuse = () => {
    if (!staticAllowed) throw new Error(staticRefusalMessage());
 ...
 */
export function choosePartSize(size) {
  const n = Number(size);
  if (!Number.isFinite(n) || n <= 0) return PART_FLOOR;
  if (n > S3_MAX_OBJECT) {
    throw new Error('That file is larger than S3 can store in a single object (5 TiB).');
  }
  if (n <= PART_FLOOR * 9000) return PART_FLOOR;
  const MiB = 1024 * 1024;
  const needed = Math.ceil(n / 9000 / MiB) * MiB;   // round up to a whole MiB
  return Math.min(Math.max(needed, S3_MIN_PART), S3_MAX_PART);
}
