...
function s3ClientOpts(cfg) {
  // Normalized here as well as on save: a config stored before the save path
  // normalized it would otherwise keep throwing "Invalid URL" until someone
  // re-typed the field, and the message gives no hint that that is the fix.
  const endpoint = normalizeEndpoint(cfg.endpoint).endpoint;
  return {
    region: cfg.region || 'auto',
    endpoint: endpoint || undefined,
    forcePathStyle: !!endpoint,
    useAccelerateEndpoint: !!cfg.accelerate && !endpoint,
    credentials: { accessKeyId: cfg.accessKeyId, secretAccessKey: cfg.secretAccessKey },
    // The SDK's default (since 3.729) adds a CRC32 to every PutObject and
    // UploadPart. Presigning has no body to checksum, so the URL carries the
    // CRC32 of zero bytes (x-amz-checksum-crc32=AAAAAA==) and the bucket
    // rejects the real upload. Only send checksums an operation requires.
...
/**
 * Make an endpoint something the S3 SDK can parse.
 *
 * The SDK builds a URL from this string, so a bare host —
 * "s3.us-west-004.backblazeb2.com", which is exactly what the Backblaze
 * console shows and exactly what gets pasted — makes `new URL()` throw
 * `TypeError: Invalid URL` before a single packet leaves the server. Every
 * check that talks to the bucket then fails with a message about URLs, and
 * the bucket, the region and the key all get blamed in turn. They were fine.
 *
 * Returns { endpoint, problem, changed }. `problem` is a sentence rather than
 * an exception: the caller is usually a diagnostic that has to keep running
 * and report, not stop.
 */
export function normalizeEndpoint(raw) {
  const trimmed = String(raw ?? '').trim();
  if (!trimmed) return { endpoint: '', problem: null, changed: false };
  ...
  if (scheme && !/^https?$/i.test(scheme[1])) {
    return {
      endpoint: '',
      changed: false,
      problem: `The endpoint starts with "${scheme[1]}://". An S3 endpoint is an https URL — for example https://s3.us-west-004.backblazeb2.com.`,
    };
  }
  ...
  try {
    ...
    return {
      endpoint: '',
      changed: false,
      problem: `"${trimmed}" is not a usable endpoint — it does not parse as a URL. It should look like https://s3.us-west-004.backblazeb2.com.`,
    };
  }
  if (!u.hostname) {
    return { endpoint: '', changed: false, problem: `"${trimmed}" has no hostname.` };
  }

  const path = u.pathname.replace(/\/+$/, '');
  const endpoint = `${u.protocol}//${u.host}${path}`;
  return { endpoint, problem: null, changed: endpoint !== trimmed };
}

/**
 * Endpoint problems that need the rest of the config to see.
 *
 * The bucket pasted onto the end of the endpoint is the common one: with
 * path-style addressing the SDK appends the bucket itself, so the request
 * goes to /<bucket>/<bucket>/<key> and every object 404s while the
 * credentials, the region and the bucket name are all correct.
 */
export function endpointProblem(cfg) {
  const { endpoint, problem } = normalizeEndpoint(cfg?.endpoint);
  if (problem) return { detail: problem, fix: 'Copy the S3 endpoint from the provider console, including the https:// prefix.' };
  if (!endpoint || !cfg?.bucket) return null;
  let path = '';
  try { path = new URL(endpoint).pathname.replace(/^\/+|\/+$/g, ''); } catch { return null; }
  if (path && path.toLowerCase() === String(cfg.bucket).toLowerCase()) {
    return {
      detail: `The endpoint ends with the bucket name ("${path}"), and the bucket is configured separately.`,
      fix: `Remove "/${path}" from the endpoint. Requests are path-style, so the bucket is appended automatically and this sends them to /${path}/${path}/…`,
    };
  }
  return null;
}
...
export function sanitizeStorageSubmission(patch) {
  ...
  for (const key of STORAGE_FIELDS) {
    ...
    if (key === 'endpoint') {
      // Keep an unparseable value as typed rather than blanking it: the
      // diagnostics can then say what is wrong with what you entered, which
      // an empty field cannot.
      const n = normalizeEndpoint(str);
      str = n.problem ? str : n.endpoint;
    }
...
export function publicUrlForKey(cfg, key) {
  ...
  const endpoint = normalizeEndpoint(cfg.endpoint).endpoint;
  if (endpoint) return `${endpoint}/${cfg.bucket}/${key}`;
...
/**
 * The last segment of an object key for a file name. Keep the real filename
 * (no random prefix) so the mounted drive shows the same name as the web.
 * Sanitize to a filesystem/URL-safe set, preserving spaces, parens, dot,
 * dash, underscore. Uniqueness is handled by s3UniqueKey. Shared by uploads
 * and renames so a renamed object is named the way an upload would be.
 */
...
/**
 * An object's checksum and length, from a HEAD: { etag, size }, or null when
 * there is no such object (or no bucket). The ETag, quotes stripped, is what
 * `content_hash` holds — the bucket's own fingerprint of the bytes: the MD5
 * for a single-part upload, an MD5 of the parts' MD5s ("…-N") for a
 * multipart one. Equal ETags and sizes mean the same bytes; the converse does
 * not hold (the same file uploaded in different part sizes gets two ETags),
 * so duplicates found this way are certain and some are missed, never the
 * other way round.
 */
export async function s3HeadObject(cfg, key) {
  if (!s3Ready(cfg) || !key) return null;
  let S3Client, HeadObjectCommand;
  try { ({ S3Client, HeadObjectCommand } = await import('@aws-sdk/client-s3')); } catch { return null; }
  const client = new S3Client(s3ClientOpts(cfg));
  try {
    const r = await client.send(new HeadObjectCommand({ Bucket: cfg.bucket, Key: key }));
    return { etag: etagOf(r.ETag), size: r.ContentLength != null ? Number(r.ContentLength) : null };
  } catch {
    return null;
  }
}

/** `"d41d8cd9…"` → `d41d8cd9…`, lowercased; null for anything else. */
export function etagOf(raw) {
  const v = String(raw || '').trim().replace(/^W\//, '').replace(/^"+|"+$/g, '').toLowerCase();
  return /^[0-9a-f]{32}(-\d+)?$/.test(v) ? v : null;
}
 ...
 */
export async function s3PresignPut(cfg, { filename, contentType, folder, cacheControl }) {
  if (!s3Ready(cfg)) throw new Error('Custom bucket isn’t fully configured.');
  ...
  const cmd = new PutObjectCommand({
    Bucket: cfg.bucket, Key: key, ContentType: contentType || 'application/octet-stream',
...
/**
 * Copy an object within the bucket, leaving the source in place. A folder
 * rename copies everything first and deletes the originals only once the
 * catalog has moved, so a failure part-way leaves the old tree whole.
 */
export async function s3CopyObject(cfg, fromKey, toKey) {
  if (!s3Ready(cfg) || !fromKey || !toKey || fromKey === toKey) return false;
...
// Imported once per process. The SDK is an external package, so webpack turns
// each import() into a fresh namespace object with a getter per export —
// over 700 for client-s3 — and presignFileUrls signs two URLs per file.
let presignSdk = null;
function loadPresignSdk() {
  presignSdk ||= Promise.all([import('@aws-sdk/client-s3'), import('@aws-sdk/s3-request-presigner')])
    .then(([{ S3Client, GetObjectCommand }, { getSignedUrl }]) => ({ S3Client, GetObjectCommand, getSignedUrl }))
    .catch((e) => { presignSdk = null; throw e; });
  return presignSdk;
}

// SigV4 refuses a presigned URL that lives longer than seven days.
 ...
 *
 * `stableFor` (seconds) signs as of the start of the current window of that
 * length, so every request inside the window gets the same URL and the
 * browser can cache what it fetched. Signing as of now changed X-Amz-Date,
 * and so the URL, every second: a thumbnail was downloaded again on every
 * page load. The expiry is stretched by the window, so a URL is still good
 * for at least `expiresIn` after it is handed out.
 */
export async function s3PresignGet(cfg, key, { expiresIn = 3600, stableFor = 0, client = null, download = false, filename = null } = {}) {
  if (!s3Ready(cfg)) throw new Error('Custom bucket isn’t fully configured.');
  let S3Client, GetObjectCommand, getSignedUrl;
  try {
    ({ S3Client, GetObjectCommand, getSignedUrl } = await loadPresignSdk());
  } catch {
  ...
  if (!stableFor) return getSignedUrl(c, new GetObjectCommand(cmdOpts), { expiresIn });
  const windowMs = stableFor * 1000;
  return getSignedUrl(c, new GetObjectCommand(cmdOpts), {
    signingDate: new Date(Math.floor(Date.now() / windowMs) * windowMs),
    expiresIn: Math.min(expiresIn + stableFor, MAX_PRESIGN_SECONDS),
  });
}

const ORIGINAL_URL_WINDOW = 3600;
const THUMB_URL_WINDOW = 86400;
...
export async function presignFileUrls(files, { expiresIn } = {}) {
  try {
    ...
    const { S3Client } = await loadPresignSdk();
    const client = new S3Client(s3ClientOpts(cfg));
    return await Promise.all(files.map(async (f) => {
      ...
      const out = { ...f };
      if (key) { try { out.url = await s3PresignGet(cfg, key, { client, stableFor: ORIGINAL_URL_WINDOW, ...(expiresIn ? { expiresIn } : {}) }); } catch {} }
      // Sign the small grid thumbnail too (so the private bucket serves it).
      // A thumbnail never changes under its key, so its URL can stay the
      // same all day and the browser serves it from cache.
      const tkey = thumbnailKeyToSign(cfg, f);
      if (tkey) {
        try { out.thumbnailUrl = await s3PresignGet(cfg, tkey, { client, expiresIn: THUMB_URL_WINDOW, stableFor: THUMB_URL_WINDOW }); } catch {}
      } else if (f.thumbnailKey || f.thumbnailUrl) {
        out.thumbnailUrl = null;
      }
      // The hover-scrub sprite sheet, on the same long window as a thumbnail
      // and for the same reason: it never changes under its key, so a stable
      // URL lets the browser serve it from cache instead of refetching a
      // sheet per page view.
      if (f.filmstripKey && isThumbnailKey(f.filmstripKey)) {
        try { out.filmstripUrl = await s3PresignGet(cfg, f.filmstripKey, { client, expiresIn: THUMB_URL_WINDOW, stableFor: THUMB_URL_WINDOW }); } catch {}
      }
      // A video's player poster, likewise. Only rows read whole carry the key
      // (the detail and share pages); the listing does not select it, since
      // no tile shows it.
      if (f.posterKey && isThumbnailKey(f.posterKey)) {
        try { out.posterUrl = await s3PresignGet(cfg, f.posterKey, { client, expiresIn: THUMB_URL_WINDOW, stableFor: THUMB_URL_WINDOW }); } catch {}
      }
      return out;
    }));
  } catch {
...
/**
 * The key a row's thumbnail is signed under, or null. Only ever one of our
 * thumbnails: presignFileUrls signs whatever this returns, so a row whose
 * thumbnail points at anything else — written through PATCH before it
 * stopped taking one — must not become a download link for that object,
 * which could be another drive's file. keyFromUrl takes the path of any URL
 * whatever its host, so the URL form needs the same check as the key.
 */
export function thumbnailKeyToSign(cfg, f) {
  const key = f?.thumbnailKey || (f?.thumbnailUrl ? keyFromUrl(cfg, f.thumbnailUrl) : null);
  return key && isThumbnailKey(key) ? key : null;
}
 ...
 *
 * `staticAllowed` says whether falling back to 'static' — the deployment's
 * own key — is permitted. Only for an admin, and never for a viewer. A static
 * key cannot be scoped or expired by IAM: it reaches every drive in the
 * bucket, and every other bucket the key can see. An admin is owner of all of
 * that already; anyone else would be handed far more than their grant. So a
 * non-admin on a provider that cannot mint a per-drive key (R2, MinIO,
 * Spaces, or AWS/B2 whose key may not mint one) is refused, and the desktop
 * app is read-only for them there — the Mac app streams through presigned
 * URLs, which need no key at all. Refusing with an actionable error is the
 * correct outcome, not a degraded one.
 *
 * For 'filespace-static' it says whether the drive's own key may be handed
 * out: to anyone whose mount role writes, and to no one who may only read.
 */
export function credentialPlan(cfg, filespace, { role = 'viewer', isAdmin = false } = {}) {
  const writes = role === 'editor' || role === 'owner';
  const staticAllowed = !!isAdmin && writes;
  const provider = storageProvider(cfg);

  // A per-filespace key is scoped to its bucket by provisioning, and we
  // cannot narrow it further: it is handed out whole or not at all. It is
  // provisioned for the drive's writers — so it goes to someone who may
  // write here (after the platform role's ceiling and the quota, which is
  // what `role` already reflects: mountRole), and to nobody else. A Viewer,
  // or anyone at their quota, would otherwise mount read-write whatever the
  // web says; they read through the app's streaming instead.
  if (filespace && filespace.accessKeyId && filespace.secretAccessKey) {
    return { strategy: 'filespace-static', staticAllowed: writes };
  }
  ...
  //
  // `staticAllowed` still follows the admin rule. It is the fallback when
  // minting fails — usually because the configured key lacks the writeKeys
  // capability — and only an admin may degrade to the master key.
 ...
 */
export function staticRefusalMessage(cfg, filespace = null) {
  if (filespace && filespace.accessKeyId && (filespace.secretAccessKey || filespace.hasSecret)) {
    return 'This drive has its own storage key, which can\u2019t be narrowed to read-only, so the desktop app can\u2019t mount it for you with view-only access. The app can still open this drive\u2019s files read-only.';
  }
  const provider = storageProvider(cfg);
  if (provider === 'b2') {
    return 'This deployment\u2019s B2 key can\u2019t mint keys scoped to one drive (it lacks the writeKeys capability), so the desktop app can\u2019t mount this drive for you. Ask an admin for a key with writeKeys.';
  }
  if (provider !== 'aws') {
    return 'This storage provider can\u2019t issue keys scoped to one drive, so mounting is for admins only here. The desktop app can still open this drive\u2019s files read-only; ask an admin for a drive with its own key to mount it.';
  }
  return 'This deployment\u2019s access key can\u2019t mint credentials scoped to one drive (no STS access), so the desktop app can\u2019t mount this drive for you. Ask an admin to allow sts:GetFederationToken on the key.';
}

export async function mintFilespaceCredentials(cfg, filespace, opts = {}) {
  const role = opts.role || 'viewer';
  const { strategy, staticAllowed } = credentialPlan(cfg, filespace, { role, isAdmin: !!opts.isAdmin });
  ...
  if (strategy === 'filespace-static') {
    if (!staticAllowed) throw new Error(staticRefusalMessage(cfg, filespace));
  ...
  if (strategy === 'b2-native') {
    try {
      ...
      if (/writeKeys|unauthorized|not authorized|401|403/i.test(String(e?.message || ''))) {
        console.warn('[mintFilespaceCredentials] B2 key minting denied — static fallback (admins only):', e.message);
  ...
  try {
    ...
    if (/AccessDenied|not authorized/i.test(text)) {
      console.warn('[mintFilespaceCredentials] GetFederationToken denied \u2014 static fallback (admins only):', e?.message);
      return staticOrRefuse();
    }
    throw e;
  }
...
export function buildMountInstaller(cfg, brand = {}) {
  ...
  const v = {
    ...
    ACCESS_KEY: cfg.accessKeyId || '', SECRET_KEY: cfg.secretAccessKey || '', ENDPOINT: normalizeEndpoint(cfg.endpoint).endpoint,
...
export async function s3Diagnostics(cfg, { origin = '' } = {}) {
  ...
  // Before anything touches the network: an endpoint the SDK cannot parse.
  // This failed as `TypeError: Invalid URL` from inside the read check, which
  // reads as a bug in Onyx rather than as a missing "https://" in a field.
  const badEndpoint = endpointProblem(cfg);
  if (badEndpoint) {
    checks.push(fail('endpoint', 'Endpoint', badEndpoint.detail, badEndpoint.fix));
  } else {
    const normalized = normalizeEndpoint(cfg.endpoint);
    if (normalized.changed) {
      checks.push(warn('endpoint', 'Endpoint',
        `Using "${normalized.endpoint}" — the saved value was "${cfg.endpoint}".`,
        'Press Save to store the corrected form. Everything works meanwhile; this is only tidiness.'));
    }
  }
  ...
  // 5. Scoped credentials — whether anyone but an admin can mount at all.
  ...
  if (plan.strategy === 'b2-native') {
    try {
      ...
      if (caps.canMintKeys) {
        ...
        checks.push(warn('scoped', 'Scoped credentials', 'This B2 key lacks the writeKeys capability.',
          'Admins can still mount with the key itself, but nobody else can mount. Create a key with writeKeys via b2_create_key to fix it.'));
      }
      if (caps.namePrefix) {
      ...
      checks.push(warn('scoped', 'Scoped credentials', `Could not check the B2 key: ${e.message}`,
        'Mounting for anyone but admins may not work. Verify the key with b2_authorize_account.'));
    }
    ...
    checks.push(warn('scoped', 'Scoped credentials', `${limits.label} has no scoped-credential API wired up.`,
      'Only admins can mount: the deployment key reaches every drive, so nobody else is given it. Give a drive its own dedicated key to let its members mount it.'));
  }
...
function describeS3Error(e, provider) {
  ...
  // The SDK parses the endpoint into a URL before it opens a socket, so this
  // is a malformed endpoint field and nothing to do with the bucket or key.
  // Unnamed, it sends people to check credentials that were never the problem.
  if (/Invalid URL/i.test(text)) {
    return 'The endpoint is not a valid URL, so no request was made. It usually needs the "https://" prefix.';
  }
