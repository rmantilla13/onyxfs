...
/** Frames in a sheet. 40 at 160px wide is a ~200KB WebP — small enough to fetch eagerly, dense enough that a 2-minute clip lands within 3 seconds per frame. */
export const FILMSTRIP_FRAMES = 40;
/** Tile width. Height follows the source aspect ratio. */
 ...
 */
export function filmstripLayout({ width, height, frames = FILMSTRIP_FRAMES, columns = FILMSTRIP_COLUMNS, tileWidth = FILMSTRIP_TILE_WIDTH } = {}) {
...
export function layoutFromMetadata(metadata) {
  ...
  return layout ? { ...layout, tileHeight: Number(strip.tileHeight) || layout.tileHeight,
                    sheetHeight: layout.rows * (Number(strip.tileHeight) || layout.tileHeight) } : null;
}
