 ...
 */
export async function reviewAccess(file, principal, { modify = true } = {}) {
  const [canRead, canModify] = await Promise.all([
    canAccessFile(file, principal),
    // Only the comment-level routes ask whether the file may be changed; the
    // feed, polled every few seconds, does not pay for the answer.
    modify ? canModifyFile(file, principal) : false,
  ]);
 ...
 * with 'read' and `modify: true`, and asks refused() again once it knows
 * whose comment it is.
 */
export async function openReview(fileId, action = 'read', { modify = false } = {}) {
  ...
  const [{ flags }, access] = await Promise.all([flagsForUser(email), reviewAccess(file, principal, { modify })]);
...
/**
 * Of `emails`, those who may read `file`: people who may sign in at all (an
 * admin, or an approved invite — isEmailGrantedAccess, the sign-in gate
 * itself), each then held to canAccessFile with a principal of their own, the
 * rule every read path uses. So a mention cannot reach (or confirm the
 * existence of) someone outside the file's audience.
 *
 * The first test is not implied by the second. canAccessFile answers for a
 * principal built from any string, and for an org-visible file outside every
 * drive — the default for an upload — its answer is yes: to an address that
 * was never an account, and to someone since banned. Each name kept here is
 * stored on the comment, made a watcher and sent a notification (and, once
 * email ships, mail), so an unchecked one would let anyone post text to any
 * inbox.
 *
...
export async function peopleWhoCanRead(file, emails = [], { cap = 20 } = {}) {
  ...
  for (const email of [...new Set(emails)].slice(0, cap)) {
    try {
      if (!(await isEmailGrantedAccess(email))) continue;
